...
protocol SwiftrisDelegate{
    ...
    // Involked when the game has reached a new level
...
class Swiftris {
    ...
    func removeCompletedLines() -> (linesRemoved: Array<Array<Block>>, fallenBlocks: Array<Array<Block>>){
        var removeLines = Array<Array<Block>>()
        for var row = NumRows - 1; row > 0; row-- {
            ...
            if rowOfBlocks.count == NumColumns{
                removeLines.append(rowOfBlocks)
        ...
        let pointsEarned = removeLines.count * PointsPerLine * level
        score += pointsEarned
        ...
        for column in 0..<NumColumns{
            ...
            for var row = removeLines[0][0].row - 1; row > 0; row-- {
                if let block = blockArray[column, row]{
                    var newRow = row
